...
fn main() {
    ...
    let a_Z : Vec<char> = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".chars().collect();
	...
	let intersection = set1.intersection(&set2).into_iter().collect::<String>().chars().collect::<Vec<char>>()[0];
	...
	if let [pack1, pack2, pack3] = backpack_3[..] {
	    ...
	    let intersection = intersection.intersection(&pack3_set).collect::<String>().chars().collect::<Vec<char>>()[0];
	    running_total += mapping.get(&intersection).unwrap();
	}
    }
