...
fn main() {
    ...
    let graph1 = construct_graph(
        ...
        &0,
        (0, 1, 0),
        (max_i, max_j - 1, 0).to_owned(),
    );
    let state_1 = graph1
        .iter()
        .flat_map(|(k, v)| v)
        .filter(|v| v.0 == max_i && v.1 == max_j - 1)
        .map(|v| v.2)
        .min()
        .unwrap();
    let graph2 = construct_graph(
        &blizz_cube,
        &max_i,
        &max_j,
        500,
    ...
    let state_2 = graph2
        .iter()
        .flat_map(|(k, v)| v)
        .filter(|v| v.0 == 0 && v.1 == 1)
        .map(|v| v.2)
        .min()
        .unwrap();
    let graph3 = construct_graph(
        &blizz_cube,
        &max_i,
        &max_j,
        500,
...
fn construct_graph(
    blizz_cube: &HashMap<i32, HashSet<(i32, i32)>>,
    max_i: &i32,
    max_j: &i32,
    length_of_cube: i32,
    lcf: &i32,
    ...
    for time in 0..=length_of_cube {
        ...
        let mut children = graph
            .iter()
            .flat_map(|(_, children)| children.to_owned())
            .filter(|c| c.2 == time as i32)
        ...
        for pos in children {
            ...
            if time == 2 {}
            future_vec = future_vec
                .into_iter()
                .filter(|v| {
                    (*v == (s.0, s.1, time as i32 + 1)
                        || *v == (e.0, e.1, time as i32 + 1)
                        || ((v.0 >= 1 && v.0 <= max_i - 1)
                            && (v.1 >= 1 && v.1 <= max_j - 1)
                            && !blizz_set.contains(&(v.0, v.1))))
                })
                .collect();
