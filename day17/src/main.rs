...
fn main() {
    ...
    while rock_count < 2022 {
        ...
        settled_set = settled_set
            .iter()
            .map(|r| (r.0 + rock_height + 4 - settled_height, r.1))
            .collect();
    ...
    println!(
        "Part 1: {:?}",
        settled_set.iter().map(|r| r.0).max().unwrap()
            - settled_set.iter().map(|r| r.0).min().unwrap()
    );
                ...
                if floor_set[rock_count % 5].contains(&ss) {
                    ...
                    if found_match {
                        ...
                        if ss == first_repeat && rock_count % 5 == first_rc_break % 5 {
                            ...
                            let n_repetitions =
                                (1000000000000 - (rock_count) as i64) as i64 / rc_diff as i64;
                            // height before the remainder is accounted for
                            first_track =
                                first_hc_break as i64 + (hc_diff as i64 * (n_repetitions + 1));
                            // find out the remaining number of rocks we need to simulate
                            remainder = ((1000000000000 - (rock_count + rc_diff) as i64) as i64
                                % rc_diff as i64) as usize;
                            ...
                            rock_count = ((rock_count as i64
                                + ((n_repetitions + 1) * rc_diff as i64))
                                % 5) as usize;
                            cycle_count = ((cycle_count as i64
                                + ((n_repetitions + 1) * cc_diff as i64) as i64)
                                % blow_stream.len() as i64)
                                as usize;
                            break 'outer;
                        }
                    }
    ...
    let ss_1 = settled_set.iter().map(|r| r.0).max().unwrap()
        - settled_set.iter().map(|r| r.0).min().unwrap();
    ...
    println!(
        "Part 2: {:?}",
        first_track + settled_set.iter().map(|r| r.0).max().unwrap() as i64
            - settled_set.iter().map(|r| r.0).min().unwrap() as i64
            - ss_1 as i64
    );
}
