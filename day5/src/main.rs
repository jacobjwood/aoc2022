...
fn main() {
    ...
    let pt1 = pt1.iter().map(|(_, b)| b.last().copied().unwrap()).collect::<String>().replace(|c: char| !c.is_alphanumeric(), "");
    let pt2 = pt2.iter().map(|(_, b)| b.last().copied().unwrap()).collect::<String>().replace(|c: char| !c.is_alphanumeric(), "");
    println!("Part 1: {}", pt1);
    println!("Part 2: {}", pt2);
}
...
fn parse_instructions(instructions_string: &str) -> Vec<Vec<usize>> {
    ...
    for line in instructions_string.lines() {
        let line_vec = line.split(" ").enumerate().filter(|(idx, _)| idx % 2 != 0).map(|(_, string)| string.parse::<usize>().unwrap()).collect::<Vec<usize>>();
