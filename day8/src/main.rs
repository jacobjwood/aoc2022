...
fn main() {
    ...
    let tree_array: Vec<Vec<u32>> = contents
        .split("\n")
    ...
    let mut visible_array: Vec<Vec<u32>> = tree_array
        .iter()
        .map(|x| x.iter().map(|_| 0).collect::<Vec<u32>>())
        .collect();
    ...
    for (r_idx, _) in tree_array.iter().enumerate() {
        ...
        for (c_idx, _) in tree_array[r_idx].iter().enumerate() {
            ...
            let min_down = &tree_array[r_idx + 1..]
                .iter()
                .map(|x| x[c_idx])
                .max()
                .unwrap();

            if (current_tree > min_left)
                | (current_tree > min_right)
                | (current_tree > min_up)
                | (current_tree > min_down)
            {
                visible_array[r_idx][c_idx] = 1;
            }
        }
    ...
    for (r_idx, row) in tree_array.iter().enumerate() {
        for (c_idx, _) in row.iter().enumerate() {
            ...
            let up_vec: Vec<&u32> = tree_array[..r_idx]
                .iter()
                .map(|x| &x[c_idx])
                .rev()
                .collect();
...
fn folding_func<'a>(vec_ref: &'a Vec<&u32>, current_tree: &u32) -> u32 {
    vec_ref
        ...
        .fold((true, 0), |(nb, count), tree| {
            (nb & (tree < current_tree), count + add_it(&nb))
        })
        .1
}
