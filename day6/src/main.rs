...
fn solution(buffer_stream: &str) {
    ...
    for (rightpointer, _) in buffer_stream.chars().enumerate() {
        ...
        if start_message {
            ...
            let hs_message = HashSet::<char>::from_iter(buffer_stream[leftpointer_2..=rightpointer].chars());
        ...
        let hs_begin = HashSet::<char>::from_iter(buffer_stream[leftpointer_1..=rightpointer].chars());
