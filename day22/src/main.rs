...
fn main() {
    ...
    let (state_set, wall_set, instructions) = parse_input(&contents);
...
fn parse_input(contents: &str) -> (HashSet<(i32, i32)>, HashSet<(i32, i32)>, Vec<String>) {
    ...
    let instructions = contents_vec[1]
        ...
        .filter(|c| *c != '\n')
        .collect::<String>();
    let instr_numbers = instructions
        .split(['L', 'R', 'U', 'D'])
        .map(|s| s.to_owned())
        .collect::<Vec<String>>();
    let instr_dirs = instructions
        .chars()
...
fn navigate_2(
    ...
    let cube_1_neighbors = HashMap::from([
        ("right", (2, "left")),
        ("down", (3, "top")),
        ("left", (5, "left")),
        ("up", (6, "left")),
    ]);
    let cube_2_neighbors = HashMap::from([
        ("right", (4, "right")),
        ("down", (3, "right")),
        ("left", (1, "right")),
        ("up", (6, "bottom")),
    ]);
    let cube_3_neighbors = HashMap::from([
        ("right", (2, "bottom")),
        ("down", (4, "top")),
        ("left", (5, "top")),
        ("up", (1, "bottom")),
    ]);
    let cube_4_neighbors = HashMap::from([
        ("right", (2, "right")),
        ("down", (6, "right")),
        ("left", (5, "right")),
        ("up", (3, "bottom")),
    ]);
    let cube_5_neighbors = HashMap::from([
        ("right", (4, "left")),
        ("down", (6, "top")),
        ("left", (1, "left")),
        ("up", (3, "left")),
    ]);
    let cube_6_neighbors = HashMap::from([
        ("right", (4, "bottom")),
        ("down", (2, "top")),
        ("left", (1, "top")),
        ("up", (5, "bottom")),
    ...
    while let Some(instruction) = instructions.pop() {
        ...
        'inner: for _ in 0..moves {
            ...
            if !state_set.contains(&next) {
                ...
                let (relative_row, relative_column) =
                    (position.0 - cc_diagonal.0 .0, position.1 - cc_diagonal.0 .1);
